...
def get_frequent_itemsets(baskets, min_sup):
    """Implements Apriori algorithm with join AND prune steps."""
    total_baskets = len(baskets)
    support_data = dict()

    # Step 1: Find frequent 1-itemsets (unchanged)
...
def generate_rules(support_data, min_conf):
    """Generates all high-confidence association rules from frequent itemsets."""
    rules = []
...
def write_output(support_data, rules, min_sup, min_conf):
    """Writes frequent itemsets and high-confidence rules to output.txt with exact formatting"""
    with open("output.txt", "w", encoding='utf-8') as f:
        # Write frequent itemsets header
        f.write(f"==Frequent itemsets (min_sup={min_sup*100:.0f}%)\n")
        
        # Sort itemsets by support (descending) and then by itemset length
